...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Centres de santé</h1>
                <p class="text-slate-500 text-sm mt-1">Gérez les hôpitaux, cliniques et SAMU pour la prise en charge médicale des victimes.</p>
            </div>
            <a href="{{ route('admin.hospitals.create') }}" class="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-semibold flex items-center shadow-lg shadow-rose-600/20 transition-all">
                <i class="fa-solid fa-plus mr-2"></i> Ajouter un centre
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div class="relative w-full sm:w-80">
                ...
                <input type="text" placeholder="Rechercher un centre..." class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-600/20 focus:border-rose-600 transition-all pl-10">
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Centre de santé</th>
                                ...
                                <td class="px-6 py-4">
                                    @if ($hospital->has_ambulance)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-800">
                                            <i class="fa-solid fa-truck-medical mr-1.5 text-[10px]"></i> Oui
                                        </span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-slate-500 font-medium">Aucun centre de santé n'est enregistré.</p>
                                    <p class="text-slate-400 text-sm mt-1">Cliquez sur « Ajouter un centre » pour commencer.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
