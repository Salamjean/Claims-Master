    ...
    <div class="space-y-5 mx-auto" style="max-width:1800px;">
        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h2 class="text-xl font-extrabold text-slate-800 flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-center">
                        <i class="fa-solid fa-clock-rotate-left text-emerald-500 text-sm"></i>
                    </div>
                    Historique des sinistres
                </h2>
                <p class="text-sm text-slate-500 mt-1">Tous les sinistres assignés à votre brigade.</p>
            </div>
        </div>
        ...
        @if($sinistres->isEmpty())
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
                ...
                <p class="text-sm text-slate-400">Aucun sinistre n'a encore été assigné à votre brigade.</p>
            </div>
        @else
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if($sinistre->status === 'en_attente')
                                            <a href="{{ route('gendarmerie.sinistres.constat.create', $sinistre->id) }}"
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition-colors">
