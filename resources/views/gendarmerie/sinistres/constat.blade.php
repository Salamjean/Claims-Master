...
<div class="w-full space-y-6 pb-12" x-data="gendarmerieConstatUniqueLayout()">
    ...
    <div class="w-full bg-slate-900 text-white p-6 md:p-8 rounded-3xl shadow-xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 text-emerald-300 rounded-full text-xs font-bold border border-emerald-500/30">
                <i class="fa-solid fa-shield-cat"></i> Gendarmerie Nationale — Procès-Verbal
            </div>
            ...
            <p class="text-xs md:text-sm text-slate-400 mt-1">Procès-verbal officiel d'enquête et de constatation sur le terrain.</p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <div class="px-4 py-2.5 bg-slate-800 rounded-2xl border border-slate-700 text-xs">
                ...
                <span class="font-extrabold text-emerald-400 text-sm">#{{ $sinistre->numero_sinistre ?? $sinistre->id }}</span>
            </div>
            <a href="{{ route('gendarmerie.sinistres.en_attente') }}" class="px-4 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-bold text-xs border border-slate-700 shadow-md">
    ...
    <form action="{{ route('gendarmerie.sinistres.constat.store', $sinistre->id) }}" method="POST" enctype="multipart/form-data" onsubmit="saveGendarmerieFabricSketchData()" novalidate class="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-start constat-form">
        @csrf
        ...
        <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-6">
            <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-3">
                ...
                <button type="button" @click="setStep(1)" :class="step === 1 ? 'bg-emerald-700 text-white shadow-lg shadow-emerald-700/30 font-bold scale-[1.02]' : 'bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200'" class="w-full p-4 rounded-2xl text-xs flex items-center justify-between transition-all">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs shrink-0" :class="step === 1 ? 'bg-white text-emerald-700' : 'bg-slate-200 text-slate-700'">1</div>
                        <div class="text-left">
                            <div class="font-bold text-sm">Lieu & Circonstances</div>
                            <div class="text-[10px] opacity-75">Localisation & récit</div>
                        </div>
                    </div>
                    <i class="fa-solid fa-chevron-right text-xs"></i>
                </button>

                <button type="button" @click="setStep(2)" :class="step === 2 ? 'bg-emerald-700 text-white shadow-lg shadow-emerald-700/30 font-bold scale-[1.02]' : 'bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200'" class="w-full p-4 rounded-2xl text-xs flex items-center justify-between transition-all">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs shrink-0" :class="step === 2 ? 'bg-white text-emerald-700' : 'bg-slate-200 text-slate-700'">2</div>
                        <div class="text-left">
            ...
            <div class="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6 rounded-3xl border border-slate-700 shadow-md space-y-3">
                <div class="flex items-center gap-2 text-xs font-bold text-emerald-400 uppercase tracking-wider">
                    <i class="fa-solid fa-id-card-clip"></i> Dossier Gendarmerie
                </div>
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div x-show="step === 1" x-transition.opacity class="space-y-6">
                <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-600 space-y-6">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-2xl bg-emerald-100 text-emerald-700 flex items-center justify-center text-lg font-black">1</div>
                            <div>
                        ...
                        <span class="px-4 py-1.5 bg-emerald-50 text-emerald-700 rounded-full text-xs font-black border border-emerald-200 uppercase tracking-wider">Étape 01</span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="lieu" value="{{ old('lieu', $constat->lieu ?? $sinistre->lieu ?? $sinistre->lieu_sinistre ?? '') }}" class="w-full text-xs font-bold rounded-2xl border-slate-200 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 bg-slate-50 py-3 text-slate-800" placeholder="Adresse complète, axe routier, repère GPS..." />
                        </div>
                        <div>
                            ...
                            <input type="datetime-local" name="date_heure" value="{{ now()->format('Y-m-d\TH:i') }}" class="w-full text-xs font-bold rounded-2xl border-slate-200 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 bg-slate-50 py-3 text-slate-800" />
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-600 space-y-4">
                        <div class="flex items-center gap-2 border-b border-slate-100 pb-3">
                            <i class="fa-solid fa-align-left text-emerald-600 text-base"></i>
                            <h3 class="font-extrabold text-slate-800 text-base">Description & Circonstances des Faits <span class="text-rose-500">*</span></h3>
                        </div>
                        <div>
                            <textarea name="description_faits" rows="6" class="w-full text-xs font-semibold rounded-2xl border-slate-200 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 bg-slate-50 p-4 text-slate-800 leading-relaxed" placeholder="Récit complet des événements et constatations de la gendarmerie..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-rose-600 space-y-4">
                        ...
                        <div>
                            <textarea name="dommages" rows="6" class="w-full text-xs font-semibold rounded-2xl border-slate-200 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 bg-slate-50 p-4 text-slate-800 leading-relaxed" placeholder="Relevé des chocs, débris et déformations..."></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-slate-700 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="temoins" rows="4" class="w-full text-xs font-semibold rounded-2xl border-slate-200 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 bg-slate-50 p-4 text-slate-800" placeholder="Dépositions et coordonnées des témoins..."></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-black text-slate-700 uppercase tracking-wider mb-2">Observations / Procès-verbaux d'infraction</label>
                            <textarea name="observations" rows="4" class="w-full text-xs font-semibold rounded-2xl border-slate-200 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100 bg-slate-50 p-4 text-slate-800" placeholder="Balisages, retraits de permis, contrôles d'alcoolémie..."></textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div x-show="step === 2" x-transition.opacity class="space-y-6">
                <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-600 space-y-6">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-2xl bg-emerald-700 text-white flex items-center justify-center text-xl font-black">A</div>
                            <div>
                                <h2 class="text-xl font-extrabold text-slate-800">VÉHICULE A (Partie Déclarante)</h2>
                                <p class="text-xs text-slate-500">Conducteur, permis, assurance A.</p>
                            </div>
                        </div>
                        <span class="px-4 py-1.5 bg-emerald-100 text-emerald-800 rounded-full text-xs font-black uppercase tracking-wider">Partie A</span>
                    </div>

                    <div class="space-y-4">
                        <h3 class="text-xs font-black text-emerald-700 uppercase tracking-wider flex items-center gap-2">
                    ...
                    <div class="space-y-4 pt-6 border-t border-slate-100">
                        <h3 class="text-xs font-black text-emerald-700 uppercase tracking-wider flex items-center gap-2">
                            <i class="fa-solid fa-file-contract"></i> Permis & Assurance A
                        </h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
            ...
            <!-- ÉTAPE 4 : CROQUIS INTERACTIF FABRIC.JS & CLÔTURE GENDARMERIE -->
            <div x-show="step === 4" x-transition.opacity class="space-y-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-600 space-y-4">
                        <div class="flex items-center justify-between border-b border-slate-100 pb-3 flex-wrap gap-2">
                            <div class="flex items-center gap-2">
                                <i class="fa-solid fa-pen-ruler text-emerald-600 text-base"></i>
                                <h3 class="font-extrabold text-slate-800 text-base">Croquis Interactif Gendarmerie</h3>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <button type="button" onclick="setGendarmerieSketchMode('draw')" id="gendarmerie-sketch-draw-btn" class="text-xs px-2.5 py-1 rounded-xl bg-emerald-600 text-white font-bold transition-all shadow-sm">
                                    <i class="fa-solid fa-pen"></i> Dessin
                                </button>
                                <button type="button" onclick="setGendarmerieSketchMode('select')" id="gendarmerie-sketch-move-btn" class="text-xs px-2.5 py-1 rounded-xl bg-slate-100 text-slate-600 font-bold transition-all hover:bg-slate-200">
                                    <i class="fa-solid fa-up-down-left-right"></i> Déplacer
                                </button>
                                <button type="button" onclick="deleteGendarmerieSelectedObject()" class="text-xs px-2.5 py-1 rounded-xl bg-rose-50 text-rose-600 font-bold transition-all hover:bg-rose-100">
                                    <i class="fa-solid fa-trash-can"></i>
                                </button>
                                <button type="button" onclick="clearGendarmerieFabricSketch()" class="text-xs px-2.5 py-1 rounded-xl bg-slate-100 text-slate-700 font-bold transition-all hover:bg-slate-200">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                <button type="button" onclick="addGendarmerieSketchSymbol('auto')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    🚗 Auto
                                </button>
                                <button type="button" onclick="addGendarmerieSketchSymbol('deux_roues')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    🏍️ 2 Roues
                                </button>
                                <button type="button" onclick="addGendarmerieSketchSymbol('camion')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    🚚 Camion
                                </button>
                                <button type="button" onclick="addGendarmerieSketchSymbol('pieton')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    🚶 Piéton
                                </button>
                                <button type="button" onclick="addGendarmerieSketchSymbol('feu')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    🚦 Feu
                                </button>
                                <button type="button" onclick="addGendarmerieSketchSymbol('stop')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    🛑 STOP
                                </button>
                                <button type="button" onclick="addGendarmerieSketchSymbol('balise')" class="px-2.5 py-1 bg-slate-100 hover:bg-emerald-50 hover:border-emerald-300 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 transition-all">
                                    ⚠️ Balise
                                </button>
                            </div>
                        </div>
                        ...
                        <div id="gendarmerie-sketch-wrapper" class="relative w-full h-80 border-2 border-dashed border-slate-300 rounded-2xl bg-white overflow-hidden shadow-inner">
                            <canvas id="gendarmerie-sketch-pad" class="w-full h-full cursor-crosshair"></canvas>
                        </div>
                        <input type="hidden" name="croquis_data" id="gendarmerie-croquis-data">
                    ...
                    <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-600 space-y-4">
                        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                            <div class="flex items-center gap-2">
                                <i class="fa-solid fa-camera text-emerald-600 text-base"></i>
                                <h3 class="font-extrabold text-slate-800 text-base">Photos & Pièces Jointes</h3>
                            </div>
                            <button type="button" onclick="addGendarmeriePhotoField()" class="px-3.5 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-bold hover:bg-emerald-100 transition-colors border border-emerald-200">
                        ...
                        <div class="pt-2">
                            ...
                            <div id="gendarmerie-photos-container" class="space-y-2">
                ...
                <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-600 space-y-4">
                    <div class="flex items-center gap-2 border-b border-slate-100 pb-3">
                        ...
                        <h3 class="font-extrabold text-slate-800 text-base">Gendarme Constatateur (Validation & Signature)</h3>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Nom & Prénoms du Gendarme <span class="text-rose-500">*</span></label>
                        ...
                        <div>
                            ...
                            <input type="text" name="agent_grade" value="{{ auth('user')->user()->grade }}" placeholder="Grade Gendarmerie..." class="w-full text-xs font-bold rounded-xl border-slate-200 bg-slate-50 py-2.5" />
                        </div>
                        <div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-slate-200">
                ...
                <div class="flex items-center gap-3">
                    <button type="button" @click="nextStep()" x-show="step < 4" class="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl text-xs shadow-lg transition-all flex items-center gap-2">
                        Suivant <i class="fa-solid fa-chevron-right"></i>
                    </button>

                    <button type="submit" x-show="step === 4" class="px-10 py-3.5 bg-emerald-700 hover:bg-emerald-800 text-white font-black rounded-2xl text-xs shadow-xl transition-all flex items-center gap-2 active:scale-95">
                        <i class="fa-solid fa-check-circle text-base"></i> Clôturer & Signer le Constat Gendarmerie
                    </button>
                </div>
            </div>
...
<!-- Fabric.js Gendarmerie Croquis Interactive Scripts avec synchronisation des dimensions -->
<script>
    let gendarmerieFabricSketch = null;

    const GENDARMERIE_SKETCH_SYMBOLS = {
        auto: {
            scale: 0.8,
            svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 64" width="40" height="64"><rect x="5" y="12" width="30" height="40" rx="6" fill="#047857" stroke="#1e293b" stroke-width="1.5"/><rect x="9" y="15" width="22" height="10" rx="3" fill="#a7f3d0"/><rect x="9" y="40" width="22" height="8" rx="3" fill="#a7f3d0"/><rect x="1" y="15" width="5" height="9" rx="2" fill="#1e293b"/><rect x="34" y="15" width="5" height="9" rx="2" fill="#1e293b"/><rect x="1" y="38" width="5" height="9" rx="2" fill="#1e293b"/><rect x="34" y="38" width="5" height="9" rx="2" fill="#1e293b"/></svg>`
        },
        deux_roues: {
            scale: 0.8,
        ...
        camion: {
            scale: 0.7,
            svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 90" width="50" height="90"><rect x="4" y="4" width="42" height="20" rx="4" fill="#64748b" stroke="#1e293b" stroke-width="1.5"/><rect x="8" y="7" width="34" height="11" rx="3" fill="#a7f3d0"/><rect x="3" y="28" width="44" height="58" rx="3" fill="#94a3b8" stroke="#1e293b" stroke-width="1.5"/><rect x="0" y="8" width="5" height="12" rx="2" fill="#1e293b"/><rect x="45" y="8" width="5" height="12" rx="2" fill="#1e293b"/></svg>`
        },
        pieton: {
            scale: 0.9,
    ...
    function initGendarmerieFabricSketch() {
        const wrapper = document.getElementById('gendarmerie-sketch-wrapper');
        const canvasEl = document.getElementById('gendarmerie-sketch-pad');
        ...
        if (gendarmerieFabricSketch) {
            gendarmerieFabricSketch.setWidth(width);
            gendarmerieFabricSketch.setHeight(height);
            gendarmerieFabricSketch.calcOffset();
            gendarmerieFabricSketch.renderAll();
            return;
        }
        ...
        gendarmerieFabricSketch = new fabric.Canvas('gendarmerie-sketch-pad', {
            width: width,
            height: height,
            isDrawingMode: true,
        ...
        gendarmerieFabricSketch.freeDrawingBrush.color = '#0f172a';
        gendarmerieFabricSketch.freeDrawingBrush.width = 3;
        gendarmerieFabricSketch.calcOffset();
    }

    function addGendarmerieSketchSymbol(type) {
        if (!gendarmerieFabricSketch) initGendarmerieFabricSketch();
        const sym = GENDARMERIE_SKETCH_SYMBOLS[type];
        if (!sym || !gendarmerieFabricSketch) return;

        fabric.loadSVGFromString(sym.svg, function(objects, options) {
            const group = fabric.util.groupSVGElements(objects, options);
            group.set({
                left: (gendarmerieFabricSketch.width / 2) - ((group.width * sym.scale) / 2),
                top: (gendarmerieFabricSketch.height / 2) - ((group.height * sym.scale) / 2),
                scaleX: sym.scale,
                scaleY: sym.scale,
                hasControls: true,
                hasBorders: true,
            ...
            gendarmerieFabricSketch.add(group);
            gendarmerieFabricSketch.setActiveObject(group);
            setGendarmerieSketchMode('select');
            gendarmerieFabricSketch.renderAll();
        });
    }

    function setGendarmerieSketchMode(mode) {
        if (!gendarmerieFabricSketch) initGendarmerieFabricSketch();
        if (!gendarmerieFabricSketch) return;
        const drawBtn = document.getElementById('gendarmerie-sketch-draw-btn');
        const moveBtn = document.getElementById('gendarmerie-sketch-move-btn');

        if (mode === 'draw') {
            gendarmerieFabricSketch.isDrawingMode = true;
            if (drawBtn) drawBtn.className = 'text-xs px-2.5 py-1 rounded-xl bg-emerald-600 text-white font-bold transition-all shadow-sm';
            ...
            gendarmerieFabricSketch.isDrawingMode = false;
            ...
            if (moveBtn) moveBtn.className = 'text-xs px-2.5 py-1 rounded-xl bg-emerald-600 text-white font-bold transition-all shadow-sm';
        }
    }

    function deleteGendarmerieSelectedObject() {
        if (!gendarmerieFabricSketch) return;
        const activeObj = gendarmerieFabricSketch.getActiveObject();
        if (activeObj) {
            gendarmerieFabricSketch.remove(activeObj);
            gendarmerieFabricSketch.discardActiveObject();
            gendarmerieFabricSketch.renderAll();
        }
    }

    function clearGendarmerieFabricSketch() {
        if (gendarmerieFabricSketch) {
            gendarmerieFabricSketch.clear();
            gendarmerieFabricSketch.setBackgroundColor('#ffffff', gendarmerieFabricSketch.renderAll.bind(gendarmerieFabricSketch));
        }
    }

    function saveGendarmerieFabricSketchData() {
        if (gendarmerieFabricSketch) {
            const dataInput = document.getElementById('gendarmerie-croquis-data');
            if (dataInput) dataInput.value = gendarmerieFabricSketch.toDataURL({ format: 'png' });
        }
    }

    function gendarmerieConstatUniqueLayout() {
        return {
            step: 1,
            setStep(s) {
                this.step = s;
    ...
    function addGendarmeriePhotoField() {
        const container = document.getElementById('gendarmerie-photos-container');
