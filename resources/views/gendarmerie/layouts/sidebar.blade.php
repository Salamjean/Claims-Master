<aside id="sidebar" class="flex flex-col">
    ...
    <div class="flex items-center gap-3 px-6 py-5 border-b border-white/10">
        ...
        <div class="logo-text">
            ...
            <p class="text-white/50 text-xs mt-0.5">Espace Gendarmerie</p>
        </div>
    </div>
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('gendarmerie.dashboard') }}"
            class="nav-item {{ request()->routeIs('gendarmerie.dashboard') ? 'active' : '' }}">
        ...
        <a href="{{ route('gendarmerie.sinistres.en_attente') }}"
            class="nav-item {{ request()->routeIs('gendarmerie.sinistres.en_attente') ? 'active' : '' }}">
        ...
        <a href="{{ route('gendarmerie.sinistres.historique') }}"
            class="nav-item {{ request()->routeIs('gendarmerie.sinistres.historique') ? 'active' : '' }}">
        ...
        <a href="{{ route('gendarmerie.agents.index') }}"
            class="nav-item {{ request()->routeIs('gendarmerie.agents.*') ? 'active' : '' }}">
    ...
    <div class="px-4 py-4 border-t border-white/10">
        <div x-data="{ open: false }" class="relative" @click.away="open = false">
            ...
            <button @click="open = !open"
                ...
                <div class="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center shrink-0 overflow-hidden">
                    @if(auth('user')->user()->profile_picture)
                        <img src="{{ asset('storage/' . auth('user')->user()->profile_picture) }}" class="w-full h-full object-cover">
                    @else
                        <span class="text-white text-xs font-bold">{{ strtoupper(substr(auth('user')->user()->name ?? 'G', 0, 1)) }}</span>
                    @endif
                </div>
                <div class="text-left nav-label flex-1 truncate">
                    <p class="text-white text-sm font-semibold truncate">
                        {{ auth('user')->user()->name ?? 'Gendarmerie' }}
                    </p>
                    <p class="text-white/50 text-xs truncate">Brigade</p>
                </div>
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-100"
                ...
                <a href="{{ route('gendarmerie.profile') }}"
                ...
                <form method="POST" action="{{ route('gendarmerie.logout') }}" class="w-full">
                    @csrf
                    <button type="submit"
