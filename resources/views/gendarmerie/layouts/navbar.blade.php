<header class="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-6 shrink-0 shadow-sm z-10">
    ...
    <div class="flex items-center gap-4">
        ...
        <button id="toggle-sidebar"
            class="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-emerald-600 transition-all">
            <i class="fa-solid fa-bars text-base"></i>
        </button>

        {{-- Fil d'Ariane --}}
        <div class="flex items-center gap-2 text-sm text-slate-400">
            <span>Espace Gendarmerie</span>
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2 pl-3 border-l border-slate-100">
            <div
                class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 bg-emerald-600">
                {{ strtoupper(substr(auth('user')->user()->name ?? 'G', 0, 1)) }}
            </div>
            <div class="hidden md:block">
                <p class="text-xs font-semibold text-slate-700 leading-none">
                    {{ auth('user')->user()->name ?? 'Brigade' }}</p>
