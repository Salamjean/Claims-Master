...
<div class="mx-auto" style="width: 100%;">
    ...
    <div class="relative overflow-hidden rounded-[2.5rem] mb-10 shadow-2xl animate-in" style="--delay: 0.1s">
        ...
        <div class="relative z-10 px-8 py-6 md:px-12 md:py-6 flex flex-col md:flex-row justify-between items-center gap-6">
            <div class="text-center md:text-left">
                ...
                <h1 class="text-2xl md:text-3xl font-black text-white mb-1 leading-tight">
                    Ravi de vous revoir, <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-emerald-300">{{ $user->prenom ?? $user->name }}</span>
                </h1>
            ...
            <div class="shrink-0">
                <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-4 text-center">
                    ...
                    <div class="flex items-center gap-2 justify-center mb-3">
                        <span class="text-xl font-black text-white font-mono tracking-tighter">{{ $user->code_user }}</span>
                        <button onclick="navigator.clipboard.writeText('{{ $user->code_user }}'); Swal.fire({toast:true, position:'top-end', icon:'success', title:'Copié !', showConfirmButton:false, timer:1500})" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            @if($dernierSinistre)
                <div class="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden animate-in" style="--delay: 0.2s">
                    <div class="p-8">
                        <div class="flex items-center justify-between mb-10">
                            <div>
                                <h2 class="text-xl font-black text-slate-800">Suivi de votre dossier en cours</h2>
                                <p class="text-sm text-slate-400 font-medium mt-1">Dernière mise à jour : {{ $dernierSinistre->updated_at->diffForHumans() }}</p>
                            </div>
                            <div class="flex flex-col items-end">
                                <span class="text-[10px] font-black text-blue-500 bg-blue-50 px-3 py-1 rounded-full uppercase tracking-widest border border-blue-100 mb-2">
                                    {{ $dernierSinistre->numero_sinistre }}
                                </span>
                                <span class="text-sm font-bold text-slate-700">{{ str_replace('_', ' ', $dernierSinistre->type_sinistre) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="relative py-4">
                            ...
                            <div class="absolute top-[21px] left-0 right-0 h-1 bg-slate-100 rounded-full"></div>
                                ...
                                @php
                                    ...
                                    $stepStates = [
                                        0 => true, // Déclaration Toujours OK
                                        1 => $isAssigned,
                                    ...
                                    // Détermination de l'étape "actuelle" pour l'animation/focus
                                    $currentStepIndex = 0;
                        ...
                        <div class="mt-12 flex flex-col md:flex-row items-center gap-6 p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100">
                            @if($dernierSinistre->assignedAgent)
                                <div class="flex items-center gap-4 border-r border-slate-200 pr-10">
                                    <div class="w-12 h-12 rounded-full bg-blue-100 border-2 border-white flex items-center justify-center font-black text-blue-600 shadow-sm">
                                        {{ substr($dernierSinistre->assignedAgent->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Agent sur place</p>
                            ...
                            <div class="flex-1 text-center md:text-left">
                                <p class="text-sm font-bold text-slate-600">
                                    ...
                                    @elseif($currentStepIndex == 1) 
                                        @if($isDocsComplete)
                                            Documents reçus. <span class="text-blue-600">L'agent est en route pour réaliser le constat terrain.</span>
                                        @else
                                            L'agent est en route. <span class="text-amber-600">N'oubliez pas de soumettre vos documents requis.</span>
                                        @endif
                                    @elseif($currentStepIndex == 2) 
                                        Constat terrain effectué. <span class="text-blue-600">L'agent procède actuellement à la rédaction officielle de votre document.</span>
                                    @elseif($currentStepIndex == 3) 
                                        @if(!$isDocsComplete)
                                            Le constat est rédigé. <span class="text-red-500">Action requise : Veuillez fournir vos documents pour validation par l'assurance.</span>
                                        @else
                                            Constat rédigé et documents reçus. <span class="text-blue-600">Dossier transmis pour révision.</span>
                                        @endif
                                ...
                                <div class="flex flex-wrap gap-3 shrink-0">
                                    ...
                                    <a href="{{ route('assure.sinistres.show', $dernierSinistre->id) }}" 
                                       class="px-6 py-3 bg-white hover:bg-slate-800 hover:text-white transition-all text-slate-800 font-bold text-sm rounded-xl border border-slate-200 shadow-sm flex items-center gap-2 group">
                                        ...
                                        <i class="fa-solid fa-chevron-right text-[10px] opacity-40 group-hover:opacity-100 transition-all"></i>
                                    </a>
                                </div>
                        </div>
                    </div>
            ...
            @else
                ...
                <div class="bg-white rounded-[2rem] border border-slate-100 p-12 text-center animate-in" style="--delay: 0.2s">
                    <div class="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6 text-blue-300 text-3xl">
                        <i class="fa-solid fa-shield-cat"></i>
                    </div>
                    <h2 class="text-2xl font-black text-slate-800 mb-3">Aucun sinistre en cours</h2>
                    <p class="text-slate-400 font-medium max-w-sm mx-auto mb-8">Tout est calme sur votre route. En cas de problème, notre assistance est là pour vous 24h/24.</p>
                    <a href="{{ route('assure.sinistres.create') }}" class="px-8 py-3.5 bg-blue-600 text-white font-black rounded-2xl shadow-lg shadow-blue-50/40 hover:bg-blue-700 transition-all">Déclarer maintenant</a>
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Graphique Sinistres -->
                <div class="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-xl shadow-slate-200/50 animate-in" style="--delay: 0.3s">
                    <h3 class="text-xs font-black text-slate-400 mb-6 uppercase tracking-[0.2em]">Tendances mensuelles</h3>
                ...
                <div class="bg-[#1a1b24] rounded-[2rem] p-8 text-white relative overflow-hidden group animate-in flex flex-col justify-between" style="--delay: 0.4s">
                    ...
                    <div class="flex items-center gap-4 relative z-10 mb-6">
                        <div class="w-12 h-12 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center overflow-hidden">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-blue-300/60 font-bold font-mono tracking-widest">{{ $user->code_user }}</p>
                        </div>
                    </div>

                    <a href="{{ route('assure.profile') }}" class="w-full py-3 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center gap-3 font-black text-xs hover:bg-white hover:text-slate-900 transition-all group relative z-10">
        ...
        <div class="space-y-8 h-full">
            ...
            {{-- Activités Récentes --}}
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 p-8 flex flex-col h-full animate-in" style="--delay: 0.5s">
                ...
                <div class="space-y-6 flex-1">
                    @forelse($recentActivities as $activity)
                        <div class="flex gap-4 relative group">
                            ...
                            @endif
                            <div class="w-11 h-11 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 group-hover:bg-{{ $activity['color'] }}-50 group-hover:border-{{ $activity['color'] }}-100 transition-colors">
                                <i class="fa-solid {{ $activity['icon'] }} text-slate-400 group-hover:text-{{ $activity['color'] }}-500 transition-colors"></i>
                            </div>
                            <div class="flex flex-col justify-center">
                                <p class="text-sm font-bold text-slate-700 leading-tight group-hover:text-slate-900 transition-colors">{{ $activity['title'] }}</p>
                                <p class="text-[11px] text-slate-400 font-medium mt-1">{{ \Illuminate\Support\Carbon::parse($activity['date'])->diffForHumans() }}</p>
                            </div>
                        </div>
                    @empty
                ...
                <div class="mt-10 pt-6 border-t border-slate-50">
                    <div class="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-100/50 flex items-center gap-4">
