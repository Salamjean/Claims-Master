...
@section('title', 'Examen du Dossier')
...
@section('content')
    <div class="mx-auto" style="width:80%">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <div class="flex items-center gap-2 text-sm text-slate-400 mb-2">
                    <a href="{{ route('personnel.mes-dossiers') }}" class="hover:text-slate-600 transition-colors">Mes
                        Dossiers</a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-indigo-50 rounded-2xl p-6 border border-indigo-100 shadow-sm relative overflow-hidden">
                        ...
                        <div class="relative z-10">
                            <h3 class="font-bold text-indigo-900 mb-4 flex items-center gap-2">
                                <i class="fa-solid fa-robot text-indigo-500"></i> Analyse de Claims AI
                            </h3>
                            <div class="space-y-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <p class="text-indigo-800 font-medium">
                                        {{ $sinistre->ai_analysis_report['context'] ?? 'Non fourni' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                    ...
                    <div class="mb-6 p-4 border border-slate-200 rounded-xl bg-slate-50">
                        ...
                        @else
                            <form action="{{ route('personnel.sinistres.verify_garanties', $sinistre->id) }}"
                                method="POST">
                                @csrf
                                <div class="space-y-3">
                                    <select name="est_couvert"
                                        class="w-full h-10 px-3 rounded-lg text-sm border-slate-300 focus:border-[#1d3557]"
                                        required
                                    ...
                                    <button type="submit"
                                        class="w-full py-2 bg-[#1d3557] text-white rounded-lg text-sm font-semibold hover:bg-[#152840] transition-colors">
                    ...
                    @if (in_array($sinistre->workflow_step, [
                        ...
                        <div class="p-4 border border-slate-200 rounded-xl bg-slate-50">
                            ...
                            <form action="{{ route('personnel.sinistres.assign_expert_garage', $sinistre->id) }}"
                                method="POST">
                                @csrf
                                <div class="space-y-3">
                                    <div>
                                    ...
                                    <button type="submit"
                                        class="w-full py-2 bg-[#457b9d] text-white rounded-lg text-sm font-semibold hover:bg-[#1d3557] transition-colors">
                                        Assigner
                                    </button>
                                </div>
                            </form>
            ...
            {{-- Colonne droite : Documents & Décision --}}
                                    ...
                                    <div
                                        ...
                                        @if ($isUploaded && $dernierSoumis && $doc->type_champ === 'file' && $dernierSoumis->file_path)
                                            ...
                                            @else
                                                ...
                                                <a href="{{ Storage::url($dernierSoumis->file_path) }}" target="_blank"
                                                    class="text-[10px] text-[#457b9d] font-bold uppercase hover:underline">Ouvrir
                                                    PDF</a>
                                            @endif
                                    ...
                                    <div class="flex-1 flex flex-col justify-between">
                                        ...
                                        @if ($isUploaded && $dernierSoumis)
                                            ...
                                            <div
                                                ...
                                                <div
                                                    class="text-xs font-medium {{ $managerOverride ? 'text-[#1d3557]' : 'text-slate-400' }}">
                                                ...
                                                <form
                                                    action="{{ route('personnel.sinistres.review-doc', ['sinistre' => $sinistre->id, 'documentAttendu' => $doc->id]) }}"
                    ...
                    <div
                        ...
                        @if ($tousDocumentsValides)
                            <form id="form-cloture-sinistre-{{ $sinistre->id }}"
                                action="{{ route('personnel.sinistres.decision', $sinistre->id) }}" method="POST">
                                @csrf
