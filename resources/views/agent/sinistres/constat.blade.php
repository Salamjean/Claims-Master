...
@section('content')
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="lieu" class="input" placeholder="Ex: Avenue Habib Bourguiba, Tunis" required>
                        </div>
                        <div>
    ...
    <script>
        ...
        if (canvas) {
            ...
            canvas.onmousedown = e => { drawing = true; const p = getPos(e); lastX = p.x; lastY = p.y; };
            canvas.onmousemove = e => { if (!drawing) return; const p = getPos(e); ctx.beginPath(); ctx.moveTo(lastX, lastY); ctx.lineTo(p.x, p.y); ctx.stroke(); lastX = p.x; lastY = p.y; };
            canvas.onmouseup = () => { drawing = false; document.getElementById('croquis-data').value = canvas.toDataURL(); };
            canvas.onmouseleave = () => drawing = false;

            // Touch events
            canvas.ontouchstart = e => { e.preventDefault(); drawing = true; const p = getPos(e); lastX = p.x; lastY = p.y; };
            canvas.ontouchmove = e => { e.preventDefault(); if (!drawing) return; const p = getPos(e); ctx.beginPath(); ctx.moveTo(lastX, lastY); ctx.lineTo(p.x, p.y); ctx.stroke(); lastX = p.x; lastY = p.y; };
            canvas.ontouchend = () => { drawing = false; document.getElementById('croquis-data').value = canvas.toDataURL(); };
        }
