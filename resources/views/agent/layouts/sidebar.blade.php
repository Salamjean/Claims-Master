<aside id="sidebar" 
    class="flex flex-col shrink-0 bg-slate-900 border-r border-white/5 shadow-2xl relative"
    ...
    <nav class="flex-1 px-3 py-6 space-y-1.5 overflow-y-auto custom-scrollbar">
        ...
        <p class="nav-label text-white/20 text-[10px] uppercase tracking-[0.2em] font-black px-4 mt-8 mb-3 whitespace-nowrap">Gestion des dossiers</p>
    ...
    <div class="p-4 border-t border-white/10 bg-black/10">
        <div class="flex items-center gap-3 w-full p-2 rounded-xl transition-all">
            <div class="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center shrink-0 overflow-hidden shadow-lg shadow-blue-500/20 border-2 border-white/10">
            ...
            <div class="text-left nav-label flex-1 truncate transition-all duration-300">
                <p class="text-white text-xs font-black truncate leading-none mb-1">{{ auth('user')->user()->name }}</p>
            ...
            <form method="POST" action="{{ route('agent.logout') }}" class="nav-label shrink-0">
                @csrf
                <button type="submit" class="w-8 h-8 rounded-lg flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Déconnexion">
