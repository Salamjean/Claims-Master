...
{
    /**
     * Affiche le tableau de bord Police
     */
    public function dashboard()
    {
        ...
        return view('police.dashboard', compact('user', 'sinistres', 'total', 'enAttente', 'enCours', 'cloture'));
    }
        ...
        $sinistres = Sinistre::whereInvolved($user->id)
            ...
            ->map(fn($s) => [
                ...
                'agent' => $s->assignedAgent ? [
                    'fullname' => $s->assignedAgent->name . ' ' . ($s->assignedAgent->prenom ?? ''),
                    'contact' => $s->assignedAgent->contact ?? '—'
                ] : null,
            ]);
    ...
    /**
     * Sinistres en attente assignés à ce service
     */
    public function enAttente()
    {
        ...
        $sinistres = Sinistre::whereInvolved($user->id)
            ...
            ->latest()
            ->get();
        return view('police.sinistres.en_attente', compact('sinistres'));
    }

    /**
     * Historique de tous les sinistres assignés à ce service
     */
    public function historique()
    {
        ...
        $sinistres = Sinistre::whereInvolved($user->id)
            ...
            ->latest()
            ->get();
        return view('police.sinistres.historique', compact('sinistres'));
    }
    ...
    {
        $user = auth('user')->user();
        // Vérification de l'implication via whereInvolved sur une instance spécifique
        abort_unless(Sinistre::where('id', $sinistre->id)->whereInvolved($user->id)->exists(), 403);
        
        $sinistre->load('assure', 'constat');
        return view('police.sinistres.sinistre_show', compact('sinistre'));
    }
    ...
    public function storeConstat(Request $request, Sinistre $sinistre)
    {
        ...
        return redirect()->route('police.sinistres.en_attente')
    ...
    public function showConstat(Sinistre $sinistre)
    {
        ...
        return view('police.sinistres.constat_show', compact('sinistre', 'constat', 'isAccident'));
    }
